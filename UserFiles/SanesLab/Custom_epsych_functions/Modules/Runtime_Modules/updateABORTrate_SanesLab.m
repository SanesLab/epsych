function handles = updateABORTrate_SanesLab(handles,variables,ABOind,f)
%handles = updateABORTrate_SanesLab(handles,variables,ABOind,ALLind,f)
%
...
%   ALLind: numerical indices (non logical) for ALL trials
%   f: handle for the gui figure
%
%Written by JY & NP 10.4.2018
...
currentdata = [variables,ABOind];
idx         =   currentdata(:,end) == 1;
%Pull out just the ABORT trials
% ABOtrials = currentdata(idx,:);
ABOtrials = currentdata;
...
switch grpstr{grpval}
    case 'None'
        ...
        if ~isempty(ABOtrials)
            
            %Calculate the overall FA rate
            FArate = 100*(sum(ABOtrials(:,end))/numel(ABOtrials(:,end)));
    ...
    otherwise
        ...
        for i = 1:numel(grps)
            ...
            grp_data = ABOtrials(ABOtrials(:,grp_ind) == grps(i),:);
